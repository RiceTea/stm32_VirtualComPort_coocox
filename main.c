...
# define BUTON_A0_READ  !(((GPIOA->IDR)&(1<<0)) - (1<<0))
# define PIN_A0_SET_INPUT  {GPIOA->MODER&=~ (1);GPIOA->MODER&=~ (1<<1);}
...
int main(void)
{
	...
	PIN_A0_SET_INPUT;  //PA0 butonunu input yapalim
	...
	while (1)
	{
		 if(BUTON_A0_READ)
		 {
			 i=0; while(BUTON_A0_READ){i++; if(i>1500)i=1500;}
			 if(i>=1500){
				 VCP_send_str("www.GokhanBeken.com\r\n");
				 i=0;
			 }
		 }
